#!/usr/bin/env python3
"""Restore Finder icons from previews embedded in SketchUp model files."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Restore Finder icons from thumbnails stored inside SketchUp models."
    )
    parser.add_argument("folder", type=Path, help="Folder to process, including its subfolders")
    parser.add_argument("--apply", action="store_true", help="Set Finder icons (default: report only)")
    ...
    helper = Path(__file__).with_name("set_finder_icon.js")
    if args.apply and not helper.is_file():
        parser.error("The readable Finder-icon helper is missing.")
    ...
    with tempfile.TemporaryDirectory(prefix="skp-icon-repair-") as temporary:
        ...
        for model in models_below(root):
            ...
            if thumbnail is None:
                print(f"No embedded preview: {model}", flush=True)
                continue
            recovered += 1
    ...
    print(f"Scanned {found} .skp file(s); recovered {recovered} embedded preview(s).", flush=True)
    if args.apply:
